...
on `for in` loops for more details on when to use
...
in the section on `for in` loops.
...
ECMAScript 3 and older, this is done using the `hasOwnProperty`
...
> automatic insertion of semicolons, can lead to subtle errors.
...
> has objects for mapping keys to values. And while associative
